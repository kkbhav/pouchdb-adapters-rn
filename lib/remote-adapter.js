...
function getRNFetch(opts, options) {
  ...
  if (options.file || options.base64) {
    ...
    fetchFun = new RNFetchBlob.polyfill.Fetch(config);
  } else if (!fetchFun) {
    fetchFun = new RNFetchBlob.polyfill.Fetch({
      indicator: true,
      IOSBackgroundTask: true,
...
function HttpPouch(opts, callback) {
  ...
  api._changes = function (opts) {
    ...
    var fetchData = function (since, callback) {
      ...
      // Get the changes
      setup().then(function () {
        var r = fetchJSON(url, fetchOpts, callback);
        controller.abort = r.cancel.bind(r);
        return r;
      }).catch(callback);
    };
