...
function getFileName(digest, contentType) {
  ...
  var extension = mime.extension(contentType) || contentType.substring(contentType.lastIndexOf('/') + 1);
  return name.concat('.', extension);
}

function getFileNameUsingExtension(digest, extension) {
  digest = digest.replace('md5-', '');
  var name = new Buffer(digest, 'base64').toString('hex');
  return name.concat('.', extension);
}
...
function deleteFile(filePath) {
  return new Promise((resolve) => {
    if (filePath) {
      RNFetchBlob.fs.unlink(filePath).then(resolve).catch(function () {
...
function saveRNAttachment(dir, data, digest, contentType, callback) {
  ...
  isFileString(data, function (isFile) {
    if (isFile) {
      const extension = getExtensionFromUri(data);
      const name = extension ? getFileNameUsingExtension(digest, extension) : getFileName(digest, contentType);
      const path = dir.concat(name);
      if (path === data) {
        callback(null, name);
      } else {
      ...
      const name = getFileName(digest, contentType);
      const path = dir.concat(name);
      ...
      stats.forEach((file) => {
        if (file && file.type === 'file' && file.filename && file.filename.startsWith('tmpFile_')) {
          if (file.path.endsWith(file.filename)) {
            ...
            promises.push(deleteFile(filePath).catch(() => Promise.resolve()));
          } else {
