...
function compactRevs(revs, docId, tx, api) {
      ...
      tx.executeSql(sql, seqs, function (tx) {
        ...
        tx.executeSql(sql, digestsToCheck, function (tx, res) {
          ...
          digestsToCheck.forEach(function (digest) {
            ...
            tx.executeSql('SELECT body FROM ' + ATTACH_STORE + ' WHERE digest=?', [digest], function (tx, res) {
              for (var i = 0; i < res.rows.length; i++) {
                var body = res.rows.item(i).body;
                api.fileManager.deleteAttachment(body);
              }
              tx.executeSql(
...
function websqlBulkDocs(dbOpts, req, opts, api, db, websqlChanges, callback) {
  ...
  var docInfos = userDocs.map(function (doc) {
    if (doc._id && RNPouchUtils.isLocalId(doc._id)) {
      return doc;
    }
    var newDoc = RNPouchUtils.parseDoc(doc, newEdits, dbOpts);
    return newDoc;
  });
  ...
  function writeDoc(docInfo, winningRev, winningRevIsDeleted, newRevIsDeleted,
    ...
    function dataWritten(tx, seq) {
      ...
      if (revsToCompact.length) {
        compactRevs(revsToCompact, id, tx, api);
      }
  ...
  function websqlProcessDocs() {
    RNPouchUtils.processDocs(dbOpts.revs_limit, docInfos, api, fetchedDocs, tx,
  ...
  function fetchExistingDocs(callback) {
    ...
    docInfos.forEach(function (docInfo) {
      if (docInfo._id && RNPouchUtils.isLocalId(docInfo._id)) {
  ...
  api.fileManager.preprocessAttachments(docInfos, 'base64', function (err) {
    if (err) {
      return callback(err);
    }
...
function openDBSafely(opts) {
  try {
    return {
      ...
      fileManager: new RNPouchUtils.FileManager(opts.name),
    };
  } catch (err) {
    return {
      error: err
...
function fetchAttachmentsIfNecessary(doc, opts, api, txn, cb) {
  ...
  function fetchAttachment(doc, att) {
    ...
    var attOpts = {path: opts.path, ctx: txn};
    api._getAttachment(doc._id, att, attObj, attOpts, function (_, data) {
      doc._attachments[att] = pouchdbUtils.assign(
        pouchdbUtils.pick(attObj, ['digest', 'content_type', 'revpos', 'length']),
        { data: data }
      );
      checkDone();
    });
...
function WebSqlPouch(opts, callback) {
  ...
  // To detect that this adapter is being used
  api._rn_adapter = true;

  api.fileManager = openDBResult.fileManager;
  api.getDBFileDir = api.fileManager.getDBFileDir;
      ...
      tx.executeSql('ALTER TABLE ' + DOC_STORE +
        ...
        tx.executeSql(sql, [], function (tx, result) {
          ...
          for (var i = 0; i < result.rows.length; i++) {
            ...
            if (RNPouchUtils.isDeleted(metadata)) {
              deleted.push(seq);
            }
            if (RNPouchUtils.isLocalId(metadata.id)) {
  ...
  function onGetVersion(tx, dbVersion) {
    if (dbVersion === 0) {
      ...
      var attach = 'CREATE TABLE IF NOT EXISTS ' + ATTACH_STORE +
        ' (digest UNIQUE, escaped TINYINT(1), body)';
      var attachAndRev = 'CREATE TABLE IF NOT EXISTS ' +
        ATTACH_AND_SEQ_STORE + ' (digest, seq INTEGER)';
  ...
  api._allDocs = function (opts, callback) {
    ...
    if (keys && keys.length > 900) {
      const chunkNum = (keys.length % 900) + 1;
      const chunks = [];
      for (let i = 0; i < chunkNum; i + 900) {
        chunks.push(keys.slice(i, i + 900));
      }
      const allDocResPromArr = Promise.all(chunks.map((chunk) => {
        return new Promise((resolve) => {
          const newOpts = pouchdbUtils.clone(opts);
          newOpts.keys = chunk;
          api._allDocsWithLimit(newOpts, (err, data) => {
            if (data) {
              resolve(data);
            } else {
              resolve({ total_rows: 0, rows: [], offset: 0 });
            }
          });
        });
      }));
      allDocResPromArr.then(function (allDocResArr) {
        const allDocsRes = allDocResArr.reduce(function (prev, current) {
          prev.rows = [...prev, ...current];
          prev.total_rows = current.total_rows;
          prev.offset = current.offset;
        }, { total_rows: 0, rows: [], offset: 0 });
        callback(null, allDocsRes);
      });
    } else {
      api._allDocsWithLimit(opts, callback);
    }
  };

  api._allDocsWithLimit = function (opts, callback) {
    var results = [];
    var totalRows;
    var updateSeq;
  ...
  api._getAttachment = function (docId, attachId, attachment, opts, callback) {
    ...
    tx.executeSql(sql, [digest], function (tx, result) {
      ...
      // Removing binary. Only file uri or base64 will be passed
      api.fileManager.parseAttachment(data, opts,  callback);
    });
  };
  ...
  api._destroy = function (opts, callback) {
    ...
    db.transaction(function (tx) {
      ...
      api.fileManager.destroy(function (err, done) {
        if (done) {
